...
if((!$_POST['topic_owner'])||(!$_POST['topic_title'])||(!$_POST['post_text'])){
	...
	$add_topic_sql="insert into forum_topics (topic_title,topic_create_time,
		topic_owner) values ('".$clean_topic_title."',now(), '".$clean_topic_owner."')";
	$add_topic_res=mysqli_query($mysqli,$add_topic_sql) or die (mysqli_error($mysqli));
	
	$topic_id=mysqli_insert_id($mysqli);
	
	$add_post_sql="insert into forum_posts (topic_id,post_text,post_create_time,
	post_owner) values ('".$topic_id."','".$clean_post_text."',now(), '".$clean_topic_owner."')";
